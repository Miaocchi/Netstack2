 ...
 */
template <typename Envelope> class ShardBoundedLane final {
  public:
    ...
    bool Push(Envelope &&envelope) noexcept {
        ...
        for (;;) {
            if (used > byte_capacity_ - bytes)
                return false;
            if (bytes_.compare_exchange_weak(used, used + bytes, std::memory_order_acq_rel,
        ...
        while (after > high && !high_watermark_.compare_exchange_weak(high, after, std::memory_order_relaxed,
                                                                      std::memory_order_relaxed)) {
        }
        return true;
    }
    ...
    std::size_t HighWatermark() const noexcept { return high_watermark_.load(std::memory_order_relaxed); }
...
template <typename LaneType> class DirectedShardLanes final {
  public:
    DirectedShardLanes(std::size_t shard_count, std::size_t message_capacity, std::size_t byte_capacity)
        : shard_count_(shard_count) {
        if (shard_count_ != 0 && shard_count_ > std::numeric_limits<std::size_t>::max() / shard_count_) {
        ...
        for (std::size_t source = 0; source < shard_count_; ++source) {
            for (std::size_t target = 0; target < shard_count_; ++target) {
                if (source == target)
                    continue;
                lanes_[Index(source, target)] = std::make_unique<LaneType>(message_capacity, byte_capacity);
            }
        }
    }

    LaneType *Lane(std::size_t source, std::size_t target) const noexcept {
        if (source >= shard_count_ || target >= shard_count_ || source == target)
            return nullptr;
        return lanes_[Index(source, target)].get();
    }

  private:
    std::size_t Index(std::size_t source, std::size_t target) const noexcept { return source * shard_count_ + target; }
