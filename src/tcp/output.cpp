...
bool AppendByte(std::array<std::uint8_t, 40>& options,
                std::size_t& length, std::uint8_t value) noexcept {
    if (length >= options.size()) return false;
    options[length++] = value;
    return true;
}

bool SerializeSynOptions(const TcpSynOptions& source,
                         std::array<std::uint8_t, 40>& options,
                         std::size_t& length) noexcept {
    length = 0;
    if (source.mss_present) {
        if (!AppendByte(options, length, 2) ||
            !AppendByte(options, length, 4) ||
    ...
    if (source.timestamp_present) {
        ...
        for (int shift = 24; shift >= 0; shift -= 8) {
            if (!AppendByte(options, length, static_cast<std::uint8_t>(
                    (source.timestamp_value >> shift) & 0xffu))) {
                return false;
            }
        }
        for (int shift = 24; shift >= 0; shift -= 8) {
            if (!AppendByte(options, length, static_cast<std::uint8_t>(
                    (source.timestamp_echo >> shift) & 0xffu))) {
                return false;
            }
        }
    }
    if (source.window_scale_present) {
        if (!AppendByte(options, length, 1) ||
            !AppendByte(options, length, 3) ||
            !AppendByte(options, length, 3) ||
            !AppendByte(options, length,
                        std::min<std::uint8_t>(source.window_scale, 14))) {
            return false;
        }
    }
    while ((length % 4) != 0) {
