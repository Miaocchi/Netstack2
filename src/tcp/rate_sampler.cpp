...
std::uint64_t SaturatingSub(std::uint64_t a, std::uint64_t b) noexcept { return a >= b ? a - b : 0; }

std::uint64_t SaturatingMulDiv(std::uint64_t bytes, std::uint64_t multiplier, std::uint64_t divisor) noexcept {
    if (divisor == 0)
        return 0;
...
void DeliveryRateSampler::OnPacketSent(PacketDeliveryState &pkt, std::uint64_t now_ms) noexcept {
...
RateSample DeliveryRateSampler::OnAck(const PacketDeliveryState &pkt, std::uint64_t acked_bytes, std::uint64_t now_ms,
    ...
    if (!pkt.retransmitted && now_ms >= pkt.first_sent_time_ms) {
        ...
        const std::uint64_t interval_ms = SaturatingSub(delivered_time_ms_, pkt.delivered_time_ms);

        if (interval_ms > 0) {
            const std::uint64_t delivered_delta = SaturatingSub(delivered_bytes_, pkt.delivered_bytes);

            sample.interval_ms = interval_ms;
            sample.interval_us = interval_ms * 1000;
            sample.delivery_rate_bytes_per_sec = SaturatingMulDiv(delivered_delta, 1000, interval_ms);
        }
    }
