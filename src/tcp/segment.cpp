...
TcpParseResult ParseTcpSegment(const IpAddress &source, const IpAddress &destination, const std::uint8_t *data,
                               std::size_t length, std::uint8_t ip_ecn) noexcept {
    ...
    const std::size_t header_length = static_cast<std::size_t>(data[12] >> 4) * 4;
    ...
    if (source.IsIpv4()) {
        checksum_seed =
            Ipv4PseudoHeaderSeed(source.Bytes(), destination.Bytes(), 6, static_cast<std::uint16_t>(length));
    } else {
        checksum_seed =
            Ipv6PseudoHeaderSeed(source.Bytes(), destination.Bytes(), 6, static_cast<std::uint32_t>(length));
    }
