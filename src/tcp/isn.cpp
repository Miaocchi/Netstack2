...
std::uint32_t TcpIsnGenerator::Generate(const FlowKey &flow, std::uint64_t now_ms) const noexcept {
    ...
    const std::uint32_t keyed = static_cast<std::uint32_t>(SipHash24(input, offset, secret_));
    std::uint64_t clock_tick = now_ms > std::numeric_limits<std::uint64_t>::max() / 250
                                   ? std::numeric_limits<std::uint64_t>::max()
                                   : now_ms * std::uint64_t{250};
    if (clock_tick <= last_clock_tick_ && last_clock_tick_ != std::numeric_limits<std::uint64_t>::max()) {
