...
bool SequenceOffset(std::uint32_t sequence, std::uint32_t base, std::int64_t &offset) noexcept {
...
TcpReceiveBuffer::TcpReceiveBuffer(std::size_t capacity, std::uint32_t initial_sequence,
                                   std::size_t initial_advertised_window)
    : capacity_(ValidateCapacity(capacity)), storage_(capacity_), occupancy_((capacity_ + 63) / 64, 0),
      first_undelivered_(initial_sequence), rcv_nxt_(initial_sequence),
      rcv_adv_(initial_sequence + static_cast<std::uint32_t>(initial_advertised_window == 0
                                                                 ? capacity_
                                                                 : std::min(initial_advertised_window, capacity_))) {}

std::size_t TcpReceiveBuffer::PhysicalIndex(std::size_t logical_offset) const noexcept {
    const std::size_t until_end = capacity_ - first_index_;
    return logical_offset < until_end ? first_index_ + logical_offset : logical_offset - until_end;
}
...
void TcpReceiveBuffer::SetOccupied(std::size_t physical_index, bool occupied) noexcept {
    const std::uint64_t mask = std::uint64_t{1} << (physical_index % 64);
...
std::size_t TcpReceiveBuffer::AcceptableWindow() const noexcept {
    ...
    return forward < kHalfSequenceSpace ? std::min<std::size_t>(forward, capacity_ - ready_bytes_) : 0;
}
...
void TcpReceiveBuffer::RecordAdvertisedWindow(std::size_t window) noexcept {
    ...
    const std::uint32_t candidate = rcv_nxt_ + static_cast<std::uint32_t>(bounded);
...
bool TcpReceiveBuffer::IsSequenceAcceptable(std::uint32_t sequence, std::size_t segment_length) const noexcept {
    if (segment_length > kMaximumCapacity)
        return false;
    std::int64_t begin = 0;
    if (!SequenceOffset(sequence, rcv_nxt_, begin))
        return false;
    const std::size_t window = AcceptableWindow();
    if (window == 0)
        return segment_length == 0 && begin == 0;
    if (segment_length == 0) {
...
TcpReceiveResult TcpReceiveBuffer::OnSegment(std::uint32_t sequence, const std::uint8_t *data, std::size_t length,
                                             bool psh) noexcept {
    ...
    if (!IsSequenceAcceptable(sequence, length)) {
        const std::int64_t segment_end_for_disposition = segment_begin + static_cast<std::int64_t>(length);
        result.disposition =
            segment_end_for_disposition <= 0 ? ReceiveDisposition::Duplicate : ReceiveDisposition::OutOfWindow;
        result.ack_decision = AckDecision::Immediate;
    ...
    const std::int64_t segment_end = segment_begin + static_cast<std::int64_t>(length);
    const std::int64_t window_end = static_cast<std::int64_t>(AcceptableWindow());

    if (segment_begin < 0) {
        const std::size_t before_rcv_nxt = static_cast<std::size_t>(-segment_begin);
    ...
    if (candidate_bytes == 0) {
        result.disposition = segment_end <= 0 ? ReceiveDisposition::Duplicate : ReceiveDisposition::OutOfWindow;
        result.ack_decision = AckDecision::Immediate;
        delayed_ack_pending_ = false;
        return result;
    }

    const std::size_t window_offset = static_cast<std::size_t>(accepted_begin);
    ...
    for (std::size_t i = 0; i < candidate_bytes; ++i) {
        ...
        if (!first_new_sequence_set) {
            first_new_sequence = sequence + static_cast<std::uint32_t>(result.left_trimmed + i);
    ...
    while (ready_bytes_ < capacity_ && Occupied(PhysicalIndex(ready_bytes_))) {
        ++ready_bytes_;
    }
    const std::size_t advanced = ready_bytes_ - old_ready_bytes;
    rcv_nxt_ += static_cast<std::uint32_t>(advanced);

    result.disposition = result.accepted_bytes == 0 ? ReceiveDisposition::Duplicate : ReceiveDisposition::Accepted;

    if (accepted_begin > 0) {
        const std::uint32_t trigger_sequence =
            first_new_sequence_set ? first_new_sequence : sequence + static_cast<std::uint32_t>(result.left_trimmed);
        ...
        if (trigger_offset != 0 && trigger_offset < kHalfSequenceSpace) {
            ...
            while (block_begin > ready_bytes_ && Occupied(PhysicalIndex(block_begin - 1))) {
                --block_begin;
            }
            while (block_end < capacity_ && Occupied(PhysicalIndex(block_end))) {
                ++block_end;
            }
            recent_sack_.left_edge = rcv_nxt_ + static_cast<std::uint32_t>(block_begin - ready_bytes_);
            recent_sack_.right_edge = rcv_nxt_ + static_cast<std::uint32_t>(block_end - ready_bytes_);
            recent_sack_valid_ = true;
        }
    }

    const bool out_of_order = accepted_begin > 0;
    const bool overlap = result.left_trimmed != 0 || result.duplicate_bytes != 0;
    const bool gap_fill = advanced > result.accepted_bytes;
    const bool immediate = blocked_ || out_of_order || overlap || result.right_trimmed != 0 || gap_fill || psh ||
...
TcpSackBlockList TcpReceiveBuffer::SackBlocks(std::size_t max_blocks) const noexcept {
    ...
    if (recent_sack_valid_) {
        ...
        if (offset < kHalfSequenceSpace && width != 0) {
            ...
            for (std::uint32_t i = 0; i < width; ++i) {
                if (logical_begin + i >= capacity_ || !Occupied(PhysicalIndex(logical_begin + i))) {
                    complete = false;
                    break;
                }
            }
            if (complete)
                result.blocks[result.count++] = recent_sack_;
        }
    }
    ...
    while (logical < capacity_ && result.count < limit && remaining != 0) {
        ...
        TcpSackBlock block;
        block.left_edge = rcv_nxt_ + static_cast<std::uint32_t>(block_begin - ready_bytes_);
        block.right_edge = rcv_nxt_ + static_cast<std::uint32_t>(logical - ready_bytes_);
        if (result.count != 0 && block.left_edge == result.blocks[0].left_edge &&
