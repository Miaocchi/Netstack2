...
class TcpIsnGenerator final {
  public:
    explicit TcpIsnGenerator(const std::array<std::uint64_t, 2> &secret) noexcept : secret_(secret) {}

    std::uint32_t Generate(const FlowKey &directional_flow, std::uint64_t now_ms) const noexcept;
