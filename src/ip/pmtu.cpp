...
PmtuCache::PmtuCache(std::size_t max_entries)
...
bool PmtuCache::ClampPmtu(std::uint32_t raw, std::uint8_t ip_version,
                           std::uint32_t& out) noexcept {
    if (ip_version != 4 && ip_version != 6) return false;
    ...
    if (raw < min_pmtu) raw = min_pmtu;
    if (raw > max_pmtu) raw = max_pmtu;
    out = raw;
    return true;
}
...
bool PmtuCache::IsExpired(const PmtuEntry& e, std::uint64_t now_ms) noexcept {
    ...
    if (now_ms < e.timestamp_ms) return true;
    return (now_ms - e.timestamp_ms) > e.expires_ms;
}

bool PmtuCache::NormalizeIp(const std::uint8_t* dst_ip, std::uint8_t ip_version,
                            std::uint8_t out[16]) noexcept {
    if (dst_ip == nullptr) return false;
    if (ip_version == 4) {
...
PmtuEntry* PmtuCache::Find(const std::uint8_t normalized[16],
                           std::uint8_t ip_version) noexcept {
    for (std::size_t i = 0; i < entries_.size(); ++i) {
        if (entries_[i].ip_version == ip_version &&
            std::memcmp(entries_[i].dst_ip, normalized, 16) == 0) {
            return &entries_[i];
        }
    }
    return nullptr;
...
PmtuLookupResult PmtuCache::Lookup(const std::uint8_t* dst_ip, std::uint8_t ip_version,
    ...
    if (!NormalizeIp(dst_ip, ip_version, normalized)) return result;

    for (std::size_t i = 0; i < entries_.size(); ++i) {
        const PmtuEntry& e = entries_[i];
        if (e.ip_version != ip_version) continue;
        if (std::memcmp(e.dst_ip, normalized, 16) != 0) continue;
...
void PmtuCache::InsertNew(const std::uint8_t normalized[16], std::uint8_t ip_version,
...
void PmtuCache::LowerFromIcmp(const std::uint8_t* dst_ip, std::uint8_t ip_version,
                              std::uint32_t pmtu, std::uint64_t now_ms,
                              std::uint32_t expires_ms) noexcept {
    std::uint32_t clamped;
    if (!ClampPmtu(pmtu, ip_version, clamped)) return;

    std::uint8_t normalized[16];
    if (!NormalizeIp(dst_ip, ip_version, normalized)) return;

    if (expires_ms == 0) expires_ms = kPmtuDefaultTtlMs;
...
void PmtuCache::RaiseFromProbe(const std::uint8_t* dst_ip, std::uint8_t ip_version,
                               std::uint32_t pmtu, std::uint64_t now_ms,
                               std::uint32_t expires_ms) noexcept {
    std::uint32_t clamped;
    if (!ClampPmtu(pmtu, ip_version, clamped)) return;
...
std::size_t PmtuCache::Size() const noexcept {
