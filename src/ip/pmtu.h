...
class PmtuCache {
  public:
    ...
    PmtuLookupResult Lookup(const std::uint8_t *dst_ip, std::uint8_t ip_version, std::uint64_t now_ms) const noexcept;
    ...
    void LowerFromIcmp(const std::uint8_t *dst_ip, std::uint8_t ip_version, std::uint32_t pmtu, std::uint64_t now_ms,
    ...
    /// @param expires_ms TTL for this entry (0 = use default)
    void RaiseFromProbe(const std::uint8_t *dst_ip, std::uint8_t ip_version, std::uint32_t pmtu, std::uint64_t now_ms,
                        std::uint32_t expires_ms = 0) noexcept;
  ...
  private:
    ...
    static bool ClampPmtu(std::uint32_t raw, std::uint8_t ip_version, std::uint32_t &out) noexcept;
    ...
    static bool NormalizeIp(const std::uint8_t *dst_ip, std::uint8_t ip_version, std::uint8_t out[16]) noexcept;
    ...
    void InsertNew(const std::uint8_t normalized[16], std::uint8_t ip_version, std::uint32_t pmtu, std::uint64_t now_ms,
