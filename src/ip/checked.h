...
template <typename T>
constexpr bool CheckedMul(T a, T b, T& result) noexcept {
    if (a == 0 || b == 0) {
        result = T{0};
        return true;
    }
...
/// Returns true and stores result if a+b does not overflow T; false otherwise.
template <typename T>
constexpr bool CheckedAdd(T a, T b, T& result) noexcept {
    if (b > std::numeric_limits<T>::max() - a) return false;
    result = a + b;
    return true;
}
...
class ReadCursor {
public:
    ReadCursor(const std::uint8_t* data, std::size_t size) noexcept
    ...
    bool ReadU32(std::uint32_t& out) noexcept {
        if (4 > size_ - pos_) return false;
        out = (static_cast<std::uint32_t>(static_cast<std::uint16_t>(data_[pos_]) << 8) << 16)
            | (static_cast<std::uint32_t>(data_[pos_ + 1]) << 16)
            | (static_cast<std::uint32_t>(static_cast<std::uint16_t>(data_[pos_ + 2]) << 8))
            | data_[pos_ + 3];
        pos_ += 4;
        return true;
    }
