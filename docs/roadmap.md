...
| P3C | **In progress** | P3C-01 DeliveryRateSampler + CongestionController 接口 + AIMD 适配完成，commit `aa9332e`。P3C-02 BBRv1 状态机测试完成，commit `78332bc`，TCP congestion 43/43、全量 31/31 三套构建全绿。P3C-03 per-flow pacer 完成，commit `957682f`，TCP send 57/57、全量 31/31 三套构建全绿。P3C-04 fragment reassembly → TCP input wiring 完成，commit `b6d79a8`，全量 31/31 三套构建全绿。P3C-05 HybridBdpAimd hybrid (BBR-style bandwidth estimation + AIMD loss response) 完成，ADR-006 Accepted，三套构建 37/37 全绿。P3C-06 FQ-CoDel scheduler 完成，commit `3644da2`。P3C-07 TcpSession 实现，commit `9a515a9`。P3C-08 BBR STARTUP/DRAIN 退出逻辑修正，commit `419c3db`。P3C-09 FQ-CoDel 流表线性探测修复 + shard egress 接线，commit `d9c002d`/`61bd611`。P3C-10 TcpSession 可写回调修复，commit `b2ba037`。P3C-11 TCP RX delivery 方向修正：从 `TrySend`（app→stack）改为 `OnDataReceived`（stack→app），commit `eb08e3d`，三套构建 39/39 全绿。P3C-12 BBR RTprop 带时间戳过期 min filter + R4 分片+PMTU 组合测试，commit `76d1278`。P3C-13 RFC 3168 ECN 握手协商 + 数据路径闭环（ECT/CE/ECE/CWR），commit `146a707`/`1c8360c`。P3C-14 **KCC v2.0 移植**：从 `liulilittle/kcc` `tcp_kcc.c`（commit `227a20b`，BSD/GPL 双许可）移植 geodesic 三分量估计器 + STARTUP/DRAIN/PROBE_BW FSM + LT-BW + 置信度门控 ACK 聚合补偿，新增 `KccController`（`CongestionAlgorithm::Kcc`，telemetry `kcc`）与公共 `TcpCongestionAlgorithm::Kcc` 配置，ADR-009 Accepted，TCP congestion 86/86、全量 42/42 三套构建全绿（shard_udp 偶发时序 flake 为测试既有问题，与本次无关）。P3C-15 **KCC 扩展（ADR-010）**：per-shard `KccKalmanFilter` 跨连接带宽共享（默认关）、ECN EWMA cwnd_gain 回退（默认开）、TSO headroom 改为 pacing-interval 估计、`kcc_turbo`/`kcc_ai_num`/`kcc_ecn`/`kcc_kf_enable` 进公共配置，congestion 97/97、全量 42/42 三套构建全绿。下一步: netem/真实 TUN 下 hybrid/BBR/KCC 验证、TcpSession 接入 handshake ESTABLISHED 路径 |
...
| P3C-01 (rate sampler + CC interface) | `aa9332e` | 无 |
...
| P3C-09 (FQ-CoDel linear probing + shard egress wiring) | `d9c002d` / `61bd611` | 无 |
| P3C-10 (TcpSession writable callback fix) | `b2ba037` | 无 |
| P3C-11 (TCP RX delivery direction fix) | `eb08e3d` | 无 |
| P3C-12 (BBR RTprop expiring min filter + R4 combo) | `76d1278` | 无 |
| P3C-13 (RFC 3168 ECN negotiation + data path) | `146a707` / `1c8360c` | 无 |
| P3C-14 (KCC v2.0 port, ADR-009) | `TBD` | 无 |
| P3C-15 (KCC extensions: KF/ECN/tunables, ADR-010) | `TBD` | 无 |
