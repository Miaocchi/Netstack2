...
| P4 | **In progress** | OpenPPP2 集成。P4-1 RuntimeDependencies 注入 + P4-3 IClock 替换已完成（commit `97ab0c9`，34/34 三套构建全绿）。P4-2 ISessionFactory 被动监听接入 TCP handshake 已完成（commit `0269184`，34/34 三套构建全绿）。P4-4 ITransportSession 回调接线已完成。P4-5 IEventSink flow event triggers 已完成：`TcpHandshakeEngine::EmitFlowEvent()` 在 established/closed/reset 转换时调用 `OnFlowEvent()`；`StackShard::EventLoopIteration()` 每 1000ms 发布 `MetricSnapshot`（rx_packets/dropped_packets/tcp_pcb_count/half_open_count/udp_datagrams）。TCP handshake 89/89、全量 34/34 三套构建全绿。已知限制（详见 IMPLEMENTATION_GUIDE §10.3）：初始 SYN 段 payload 被忽略。详见 `docs/plans/P4_OPENPPP2_INTEGRATION_PLAN.md`。 |
