...
std::vector<std::uint8_t> PacketBuilder::BuildIpv4Tcp(std::uint32_t src_ip, std::uint32_t dst_ip,
                                                      std::uint16_t src_port, std::uint16_t dst_port, std::uint32_t seq,
                                                      std::uint32_t ack, std::uint8_t flags,
                                                      const std::vector<std::uint8_t> &payload, std::uint16_t ip_id,
                                                      std::uint8_t ttl) {
    ...
    for (std::size_t i = 0; i + 1 < sizeof(pseudo); i += 2) {
        sum += static_cast<std::uint16_t>((static_cast<std::uint16_t>(pseudo[i]) << 8) | pseudo[i + 1]);
    }
...
ParsedPacket PacketParser::ParseIpv4Tcp(const std::vector<std::uint8_t> &bytes) {
    ...
    out.payload.assign(bytes.begin() + static_cast<std::ptrdiff_t>(tcp_off + tcp_hdr_len), bytes.end());
    return out;
}
...
std::uint16_t Ipv6TcpChecksum(const std::uint8_t src_ip[16], const std::uint8_t dst_ip[16], const std::uint8_t *tcp_seg,
                              std::size_t tcp_len) {
    std::uint32_t sum = 0;
    for (std::size_t i = 0; i < 16; i += 2) {
        sum += static_cast<std::uint16_t>((static_cast<std::uint16_t>(src_ip[i]) << 8) | src_ip[i + 1]);
    }
    for (std::size_t i = 0; i < 16; i += 2) {
        sum += static_cast<std::uint16_t>((static_cast<std::uint16_t>(dst_ip[i]) << 8) | dst_ip[i + 1]);
    }
...
std::vector<std::uint8_t> PacketBuilder::BuildIpv6Tcp(const std::uint8_t src_ip[16], const std::uint8_t dst_ip[16],
                                                      std::uint16_t src_port, std::uint16_t dst_port, std::uint32_t seq,
                                                      std::uint32_t ack, std::uint8_t flags,
...
std::vector<std::uint8_t> PacketBuilder::BuildIpv4TcpFragment(std::uint32_t src_ip, std::uint32_t dst_ip,
                                                              std::uint16_t ip_id, std::uint16_t fragment_offset,
    ...
    const std::uint16_t flags_frag = static_cast<std::uint16_t>((mf ? 0x2000 : 0x00) | (fragment_offset & 0x1FFF));
...
std::vector<std::uint8_t> PacketBuilder::BuildIpv6TcpFragment(const std::uint8_t src_ip[16],
                                                              const std::uint8_t dst_ip[16],
                                                              std::uint32_t identification,
                                                              std::uint16_t fragment_offset, bool mf,
    ...
    const std::uint16_t ff = static_cast<std::uint16_t>((fragment_offset << 3) | (mf ? 0x01 : 0x00));
    Append16(pkt, ff);
...
ParsedPacket PacketParser::ParseIpv6Tcp(const std::vector<std::uint8_t> &bytes) {
    ...
    out.is_ipv6 = true;
    if (bytes.size() < 60)
        return out; // 40 IPv6 + 20 TCP minimum
    if ((bytes[0] >> 4) != 6)
        return out;
    ...
    for (std::size_t i = 0; i < 8; ++i) {
        if (nh == 44) {
            ...
            if (offset + 2 > bytes.size())
                return out;
            nh = bytes[offset];
            const std::size_t ext_len = (static_cast<std::size_t>(bytes[offset + 1]) + 1) * 8;
            offset += ext_len;
    ...
    const std::uint8_t tcp_hdr_len = static_cast<std::uint8_t>((bytes[offset + 12] >> 4) * 4);
    if (offset + tcp_hdr_len > bytes.size())
        return out;
    ...
    out.tcp_checksum_ok = Ipv6TcpChecksum(out.src_ip6, out.dst_ip6, bytes.data() + offset, tcp_len) == 0;
    out.ip_checksum_ok = true; // IPv6 has no header checksum

    out.valid = out.tcp_checksum_ok;
    out.payload.assign(bytes.begin() + static_cast<std::ptrdiff_t>(offset + tcp_hdr_len), bytes.end());
    return out;
}
