...
#define TCPIP2_TEST_MAIN()                                                       \
    int main() {                                                                 \
        return ::tcpip2::test::TestRegistry::Instance().RunAll() == 0 ? 0 : 1;   \
    }

#define TCPIP2_EXPECT_TRUE(cond)                                                 \
    do {                                                                         \
        if (!(cond)) ::tcpip2::test::RecordFailure(__FILE__, __LINE__, #cond);   \
    } while (0)
...
#define TCPIP2_EXPECT_EQ(a, b)                                                   \
    do {                                                                         \
        ...
        if (!(tcpip2_va == tcpip2_vb)) {                                         \
            ::tcpip2::test::RecordFailure(__FILE__, __LINE__,                    \
                                          #a " == " #b " (" #a " = " #b ")");    \
        }                                                                        \
    } while (0)
