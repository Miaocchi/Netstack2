...
class ScriptedSession final : public ITransportSession {
  public:
    explicit ScriptedSession(std::vector<SendResult> script) : script_(std::move(script)) {}
...
template <std::size_t N> void ExpectView(const TcpReadyView &view, const std::array<std::uint8_t, N> &expected) {
...
TCPIP2_TEST(DelayedAckSentResetsPairing) {
    ...
    TCPIP2_EXPECT_EQ(AckDecision::Delayed, receive.OnSegment(100, byte.data(), byte.size(), false).ack_decision);
    receive.AckSent();
    TCPIP2_EXPECT_EQ(AckDecision::Delayed, receive.OnSegment(101, byte.data(), byte.size(), false).ack_decision);
}

TCPIP2_TEST(OutOfOrderThenGapFillIsImmediatelyAcknowledged) {
    TcpReceiveBuffer receive(16, 100);
    const std::array<std::uint8_t, 2> tail{{5, 6}};
    const auto out_of_order = receive.OnSegment(104, tail.data(), tail.size(), false);
    ...
    ExpectView(receive.ReadyView(), std::array<std::uint8_t, 6>{{1, 2, 3, 4, 5, 6}});
}
...
TCPIP2_TEST(SequenceWrapPreservesOrderingAndSackEdges) {
    ...
    ExpectView(receive.ReadyView(), std::array<std::uint8_t, 5>{{1, 2, 3, 4, 5}});
}
...
TCPIP2_TEST(PartialOverlapKeepsFirstArrivalBytes) {
    ...
    const auto result = receive.OnSegment(102, overlap.data(), overlap.size(), false);
    ...
    ExpectView(receive.ReadyView(), std::array<std::uint8_t, 8>{{1, 2, 3, 4, 10, 11, 12, 13}});

    const auto duplicate = receive.OnSegment(104, first.data(), first.size(), false);
...
TCPIP2_TEST(AlreadyReceivedPrefixIsTrimmed) {
    ...
    const auto result = receive.OnSegment(98, crossing.data(), crossing.size(), false);
...
TCPIP2_TEST(RightEdgeTrimAndOutOfWindowAreImmediate) {
    ...
    const auto trimmed = receive.OnSegment(100, bytes.data(), bytes.size(), false);
...
TCPIP2_TEST(ReadyViewReturnsOnePhysicalRingChunk) {
    ...
    ExpectView(receive.ReadyView(), std::array<std::uint8_t, 3>{{5, 6, 7}});
    TCPIP2_EXPECT_EQ(std::size_t{3}, receive.ConsumeReady(3));
    ExpectView(receive.ReadyView(), std::array<std::uint8_t, 3>{{8, 9, 10}});
...
TCPIP2_TEST(SackBlocksPutMostRecentTriggerFirstAndRemainBounded) {
    ...
    TCPIP2_EXPECT_EQ(std::uint32_t{124}, receive.SackBlocks().blocks[0].left_edge);
}
...
TCPIP2_TEST(DuplicateOutOfOrderSegmentBecomesFirstSackBlock) {
    ...
    TCPIP2_EXPECT_EQ(std::uint32_t{110}, receive.SackBlocks().blocks[0].left_edge);
    receive.OnSegment(104, bytes.data(), bytes.size(), false);
    const auto sacks = receive.SackBlocks();
    TCPIP2_EXPECT_EQ(std::uint32_t{104}, sacks.blocks[0].left_edge);
    TCPIP2_EXPECT_EQ(std::uint32_t{106}, sacks.blocks[0].right_edge);
...
TCPIP2_TEST(BlockedWindowIsZeroButReadyDataCanDrain) {
    ...
    receive.RecordAdvertisedWindow(receive.AdvertisedWindow());
    TCPIP2_EXPECT_EQ(ReceiveDisposition::Accepted, receive.OnSegment(103, bytes.data(), 1, false).disposition);
}
...
TCPIP2_TEST(BlockedFlowHonorsOnlyPreviouslyAdvertisedRightEdge) {
    ...
    const std::array<std::uint8_t, 4> bytes{{1, 2, 3, 4}};
    receive.SetBlocked(true);
    const auto in_flight = receive.OnSegment(100, bytes.data(), bytes.size(), false);
...
TCPIP2_TEST(NullZeroAndOversizedInputsAreInvalid) {
    ...
    const auto oversized = receive.OnSegment(50, byte.data(), std::size_t{0x80000000u}, false);
...
TCPIP2_TEST(SessionDeliveryRejectsInvalidAndZeroProgressResults) {
    ...
    DeliverFn invalid_deliver = [&invalid](BufferView data) -> SendResult { return invalid.TrySend(data); };
    TCPIP2_EXPECT_EQ(TcpDeliveryStatus::InvalidResult, DrainTcpReceiveBuffer(invalid_receive, invalid_deliver).status);
    ...
    DeliverFn stalled_deliver = [&stalled](BufferView data) -> SendResult { return stalled.TrySend(data); };
    TCPIP2_EXPECT_EQ(TcpDeliveryStatus::NoProgress, DrainTcpReceiveBuffer(stalled_receive, stalled_deliver).status);
...
TCPIP2_TEST(SessionDeliveryReportsTerminalStatusAfterAcceptedPrefix) {
    ...
    DeliverFn closed_deliver = [&closed](BufferView data) -> SendResult { return closed.TrySend(data); };
    ...
    DeliverFn error_deliver = [&error](BufferView data) -> SendResult { return error.TrySend(data); };
    TCPIP2_EXPECT_EQ(TcpDeliveryStatus::Error, DrainTcpReceiveBuffer(error_receive, error_deliver).status);
}
